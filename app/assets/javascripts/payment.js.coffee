...
payment =
  setupForm: ->
    $('#new-user').submit ->
      ...
      if $('#user_stripe_credit_card_token').val() == ''
        payment.processCard()
        false
      else
        true
  ...
  processCard: ->
    exp_date = $('#user_credit_card_expiration').val().split('/')
    card =
      number: $('#user_credit_card_number').val()
      cvc: $('#user_credit_card_cvv').val()
      expMonth: exp_date[0]
      expYear: exp_date[1]
      name: $('#user_credit_card_name').val()
    Stripe.createToken(card, payment.handleStripeResponse)

  handleStripeResponse: (status, response) ->
    if status == 200
      $('#user_stripe_credit_card_token').val(response.id)
      $('#new-user')[0].submit()
    else
      payment.addErrors(response);
      $('input[type=submit]').attr('disabled', false)

  clearErrors: ->
    $('#credit-card-errors').text('');

  addErrors: (response) ->
    payment.clearErrors();
    $('#credit-card-errors').text(response.error.message)
    $.scrollTo('#credit-card', 800)
